...
/* Use this macro with index so save a partial time. */
...
void cifar( int8_t*img, int8_t*outd);
uint8_t str[128];
uint8_t TS_status = 0;
...
typedef union
{
	int8_t image[IMAGE_RECORD_LEN];      // full array: label + raw image data
	struct
	{
...
/* Union for for single pixel representation. */
typedef union 
{
  unsigned int   val;
  ...
  LCD_Config();
	...
	for ( yt=0; yt<15; yt++)		// Image vertical position y
	{
		for ( xt=0; xt<25; xt++)	// Image orizzontal position x
		{
			...
			aRandom32bit = (aRandom32bit % 990) + 1;
    ...
    if(TS_State.touchDetected) 
		{
			...
			if( 0 == Touch) 
			{
				...
				sprintf( line, " Images correctly estimated: %d.", cifarOk);
				BSP_LCD_DisplayStringAtLine(2, (uint8_t *)line);
				sprintf( line, " Execution Time: %2.4f ms.", tdiff[cTIME_MEASURE_1]);
...
uint32_t readImage( uint32_t idx)
{
	...
	/* Apro il file binario delle imagini */
	sprintf(fn, "%d_%d.bin", img.color.label, idx);
	if (f_open(&fp, (TCHAR const*)fn, FA_READ) != FR_OK)
	{
		return 1;
	}
	/* Leggo il record con l'indice della label ed i colori. */
	f_read (&fp, binaryImage, IMAGE_SIZE, (UINT *)&BytesRead);
	f_close(&fp);
...
void plotImage( uint32_t x, uint32_t y)
{
	...
	for ( yp=0; yp<32; yp++)		// y
	{
		for ( xp=0; xp<32; xp++)	// x
		{
			/* Compongo il valore del pixel: ARGB */
