...
mean_array = mean_array.astype('int8')
for ii in range(0,(len(mean_array))):
  print ii, mean_array[ii]

out_f = open("mean_array.data", 'wb')
bin_mean_array = bytearray(mean_array)
out_f.write(bin_mean_array)
out_f.close()

out_f = open("mean_array.h", 'w')
out_f.write("#define MEAN_DATA {")
mean_array.tofile(out_f, sep=",", format="%d")
out_f.write("}\n\n")
out_f.close()
